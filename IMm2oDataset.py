...
class IMm2oDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, m_length, only_pos = True, pm_path = None, transform=None):
        ...
        # null index preprocessing
        # self.img_volumes[0] = self.img_volumes[1]
        # self.mask_volumes[0] = self.mask_volumes[1]

        for idx in range(1, len(self.img_volumes)):
            if len(self.img_volumes[idx]) == 2:
                self.img_volumes[idx] = self.img_volumes[idx-1]
                self.mask_volumes[idx] = self.mask_volumes[idx-1]
            if len(self.img_volumes[idx]) == 0:
                self.img_volumes[idx] = self.img_volumes[idx+1]
                self.mask_volumes[idx] = self.mask_volumes[idx+1]
