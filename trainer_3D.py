...
class Trainer():
    ...
    def train(self, dataloader, epoch_index=0):
        ...
        for batch_id, (x, y, filename) in enumerate(tqdm(dataloader)):
            ...
            self.loss += loss.item()
            self.logger.debug(
                f"TRAINER | train epoch: {epoch_index}, batch: {batch_id}/{len(dataloader) - 1}, loss: {loss.item()}")
    ...
    def validate(self, dataloader, epoch_index=0):
        ...
        self.loss = self.loss / len(dataloader)
        self.logger.debug(
            f"TRAINER | val/test epoch: {epoch_index}, batch: {batch_id}/{len(dataloader) - 1}, loss: {loss.item()}")
        self.logger.debug(f"TRAINER | {filename}")
