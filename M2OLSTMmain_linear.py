import os, cv2, logging, focal_loss, BinaryFocalLoss
...
from models.m2oLSTM_linear import *
...
def main(mode, gpu, exp_prefix='test', train_epoch=100,save_epoch=None, load_epoch=None, resume_exp=None, slice=1, batch_size=1, fastEXP=False):
    ...
    save_path = os.path.join("./3Dexperiment_liear/",exp_prefix)
    print(save_path)
    ...
    if fastEXP:
        ...
        # test_index_list = [(30, 44), (45, 59)]
        # test_index_list = [(0, 14),(15, 29)]
    ...
    if 'train' in mode:
        for i, test_index in enumerate(test_index_list):
            ...
            model = m2oNetBack(in_channels=2, conv_channel=64, lstm_channel=32, m_length=slice) # change 64 to 32
            # print(model)
            model.to(device)
            ...
            ### mia
                ...
                mia_pm_generator('%d%d_e35_weight.pth' %(test_index[0],test_index[1]),
                                      ...
                                      dir_path=pm_path, data_path=data_path, subjects=test_index, save_image=True)
            ...
            print("*** Train data len %d, Test data len %d ***"%(len(train_indicies),len(test_indicies)))

            train_dataset = torch.utils.data.Subset(dataset, train_indicies)
            train_dataset.dataset.transform = train_transform
            # define dataloader
            data_loader = torch.utils.data.DataLoader(
                train_dataset, batch_size=batch_size, shuffle=True, num_workers=8, collate_fn=collate_fn_pad_z, pin_memory=True)
        ...
        result_path = './pred_result/pred_unet_linear'
        ...
        if only_test:
            ...
            for test_index in test_index_list:
                ...
                model = m2oNetBack(in_channels=2, conv_channel=64, lstm_channel=32, m_length=slice)
                ...
                model_path = '%d%d_pos_LSTM_weight_e%d.pth' % (test_index[0], test_index[1], train_epoch)
                # model_path = '014_pos_LSTM_weight_e1.pth'
                ...
                for idx in test_indicies:
                    ...
                    with torch.no_grad():
                        ...
                        volume = volume.to(device) # volum to 5
                        # target = target.to(device)
...
if __name__ == '__main__':
    ...
    # main('train', exp_prefix='unet', gpu=7, train_epoch=21,
    #      save_epoch=[1,2,3,4,5,10,15,20,25,30,35,40,45,50], load_epoch=50, slice=3, batch_size=3, resume_exp=resume_exp, fastEXP=False)

    # main('train', exp_prefix='mask', gpu=3, train_epoch=31, #slice upper five
    #      save_epoch=[1,5,10,15,20,25,30,35,40,45,50], load_epoch=50, slice=5, batch_size=2, resume_exp=resume_exp, fastEXP=False)
    #

    main('eval', exp_prefix='unet', gpu=6, train_epoch=2,
         save_epoch=[5, 10, 20, 30, 40, 50, 100, 150], load_epoch=20, slice=3, batch_size=2, resume_exp=resume_exp, fastEXP=False)
