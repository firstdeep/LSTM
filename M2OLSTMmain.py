...
from pm_generator.maskrcnn_pm_generator import *
from pm_generator.mia_pm_generator import *
from pm_generator.unet_pm_generator import *
from pm_generator.uPP_pm_generator import *
...
def main(mode, gpu, exp_prefix='test', train_epoch=100,save_epoch=None, load_epoch=None, resume_exp=None, slice=1, batch_size=1, fastEXP=False):
    ...
    if 'train' in mode:
        for i, test_index in enumerate(test_index_list):
            ...
            model = m2oNetBack(in_channels=2, conv_channel=64, lstm_channel=64, m_length=slice, Device=device)
            print(model)
            model.to(device)

            # dummy_inputs = torch.rand(1, 5, 2, 512, 512)
        ...
        result_path = './pred_result/pred_mask_with'
...
if __name__ == '__main__':
    ...
    main('train', exp_prefix='mask', gpu=6, train_epoch=31,
         save_epoch=[20, 30, 40, 50, 60, 70, 80, 90, 100, 150], load_epoch=50, slice=3, batch_size=3, resume_exp=resume_exp, fastEXP=False)



    # main('eval', exp_prefix='mask', gpu=0, train_epoch=50,
    #      save_epoch=[5, 10, 20, 30, 40, 50, 100, 150], load_epoch=50, slice=3, batch_size=1, resume_exp=resume_exp, fastEXP=False)
